...
my $template = HTML::Template->new( filename => 'klepik_homework_list.html' );
my $id       = $io_cgi->param('id');

my $sth = $dbh->prepare(
"SELECT description, max_devilery_data, max_rating FROM klepiktgbot_homework_id WHERE titlegroup_id = ?"
);
$sth->execute($id);
my @homework;
...
$template->param( homework => \@homework );
$sth->finish;
$dbh->disconnect;
